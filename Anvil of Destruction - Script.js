...
function clickChar(charId) {
  ...
  document.getElementById("allBigNames").style.display = "none";
  document.getElementById("allTitanTraits").style.display = "none";
  document.getElementById("damageTable").style.display = "none";
  resetIndividualDisplay();
  
  if (charK.includes("MEGA-GARGANT") === true && charK.includes("KRAKEN-EATER") === true) {
    document.getElementById("allTitanTraits").style.display = "block";
    document.getElementById("dmgTable99check").innerHTML = "true";
    addToWS(Krakeneater_DmgTable, "damageTable", currentTab);
    showIndividualDisplay("inW_02", "missile"); }
  else if (charK.includes("TYRANT") === true) {
    document.getElementById("allCommandAbilities").style.display = "block";
    document.getElementById("allBigNames").style.display = "block";
    showIndividualDisplay("inW_01", "melee"); };
};
...
function extendedAddToWS(theNameExt, wsPartExt, theTabExt, checkIdExt) {
  ...
  if (charK.innerHTML.includes("MEGA-GARGANT") == true) {
    addToWS(Longshanks, "abilities", theTabExt);
    addToWS(Son_of_Behemat, "abilities", theTabExt);
    addToWS(Crushing_Charge, "abilities", theTabExt);
  };
  var checkTyrant = 0
  ...
  || document.getElementById("com_03").checked == true) { 
    if (document.getElementById("com_01").checked == true) { addToWS(They_Look_A_Bit_Tasty, 'command', currentTab); }
    else if (document.getElementById("com_02").checked == true) { addToWS(The_More_the_Merrier, 'command', currentTab); }
    else if (document.getElementById("com_03").checked == true) { addToWS(Flatten_Em, 'command', currentTab); };
    checkTyrant = parseInt(checkTyrant) + 1;
  };
  if (document.getElementById("bna_01").checked == true 
  || document.getElementById("bna_02").checked == true 
  || document.getElementById("bna_03").checked == true 
  || document.getElementById("bna_04").checked == true) { 
    if (document.getElementById("bna_01").checked == true) { addToWS(Mawseeker, 'abilities', currentTab); }
    else if (document.getElementById("bna_02").checked == true) { addToWS(Kin_eater, 'abilities', currentTab); }
    else if (document.getElementById("bna_03").checked == true) { addToWS(Mountain_eater, 'abilities', currentTab); }
    else if (document.getElementById("bna_04").checked == true) { addToWS(Beastkiller, 'abilities', currentTab); };
    checkTyrant = parseInt(checkTyrant) + 1;
  };
  ...
  document.getElementById("validateStep" + currentTab).innerHTML = "validated";
  if (checkTyrant <= 1 && charK.innerHTML.includes("MEGA-GARGANT") == false) { document.getElementById("validateStep" + currentTab).innerHTML = "not valid"; };
  if (checkType == "checkbox" && document.getElementById(checkIdExt).checked == false) {} 
  else if (charK.innerHTML.includes("TYRANT") == true) {} 
...
// reset/hide displayed weaponIndividualTable ------------------------------
function resetIndividualDisplay() {
  var wTables = ["melee", "missile"]
  for (w = 0; w < wTables.length; w++) {
    var wTable = document.getElementById(wTables[w] + "WeaIndividualTable")
    ...
    do {
      if (rows > 1) { 
        ind01 = wTable.rows[1].cells[0].innerHTML;
        wTable.deleteRow(1);
      };
    ...
    for (r = 1; r < rowsPre; r++) {
      if (wTablePre.rows[r].cells[0].innerHTML == ind01) { 
    ...
    document.getElementById(wTables[w] + "IndividualTable").style.display = "none";
    if (rowsPre > 1) { document.getElementById(wTables[w] + "TablePre").style.display = "block"; }
    else { document.getElementById(wTables[w] + "TablePre").style.display = "none"; };
  };  
  //document.getElementById("missileTable").style.display = "none";
  //document.getElementById("meleeTable").style.display = "none";
};

// display weaponIndividualTable ------------------------------
function showIndividualDisplay(indWea, weaArt) {
  //indWea = "inW_01"
  //weaArt = "melee"
  var inWs = [indWea];
document.getElementById("demo1").innerHTML = "Test1: "+ indWea +"!"+ weaArt +"!"+ inWs[0]
  for (i = 0; i < inWs.length; i++) {
    //var checkBox = document.getElementById(inWs[i]);
    //if (checkBox.checked == true) {
      var tableInd = document.getElementById(weaArt + "WeaIndividualTable");
      var tablePre = document.getElementById(weaArt + "WeaTablePre");
      var actRows = 0
      
      //for (b = 2; b <= 3; b++) {
        actRows = tablePre.getElementsByTagName("tr").length;
        var rowInd = tableInd.insertRow(1);
        var rowPre = tablePre.insertRow(actRows);
        
        for (n = 1; n < 8; n++) {
          cellId = "r1_" + inWs[i] + "c" + n;
          cellTxt = document.getElementById(cellId).innerHTML;
          rowInd.insertCell(n - 1).innerHTML = cellTxt;
          rowPre.insertCell(n - 1).innerHTML = cellTxt;
        };
      //};
      document.getElementById(weaArt + "IndividualTable").style.display = "block";
      document.getElementById(weaArt + "TablePre").style.display = "block";
    //};
  };
};

// reset different names to origin ------------------------------
function resetDiffNames() {
  var forms = ["commandAbilitiesForm", "bigNamesForm", "titanTraitsForm"]
...
// reset Step 2 ------------------------------
function resetStep2() {
  document.getElementById("dmgTable99").innerHTML = "false";
  document.getElementById("dmgTable99check").innerHTML = "false";
  ...
  var cBoxes = document.getElementById("bigNamesForm").getElementsByTagName("input")
  for (i = 0; i < cBoxes.length; i++) {
    if (cBoxes[i].type == 'radio') {
      cBoxes[i].checked = false;
      cBoxes[i].disabled = false;
  ...
  var cBoxes = document.getElementById("titanTraitsForm").getElementsByTagName("input")
  for (i = 0; i < cBoxes.length; i++) {
    if (cBoxes[i].type == 'radio') {
      cBoxes[i].checked = false;
      cBoxes[i].disabled = false;
...
var ohWs = ["ohW_01", "ohW_02", "ohW_03", "ohW_04", "ohW_05", "ohW_06", "ohW_07", "ohW_08"]
var oh2s = ["oh2_01"]
var thWs = ["thW_01", "thW_02", "thW_03", "thW_04", "thW_05", "thW_06", "thW_07", "thW_08"]
var ohMs = []
var thMs = []
var shis = ["shi_01", "shi_02", "shi_03"]
...
function resetWeaDisplay() {
  ...
  for (w = 0; w < wTables.length; w++) {
    ...
    do {
      if (rowsPre > 1) {
        if (!(wTablePre.rows[1].cells[0].innerHTML == document.getElementById("r1_inW_01c1").innerHTML) 
        && !(wTablePre.rows[1].cells[0].innerHTML == document.getElementById("r1_inW_02c1").innerHTML)) { 
          wTablePre.deleteRow(1); }
        else { indWea = parseInt(indWea) + 1; };
      };
      rowsPre = wTablePre.getElementsByTagName("tr").length;
    }
    while (rowsPre > (1 + parseInt(indWea)));
...
function resetRestrictionsStep3() {
  ...
  if (str.includes("OGOR MAWTRIBES") === true) {
    for (i = 5; i < ohWs.length; i++) { document.getElementById(ohWs[i]).disabled = true; };
    for (i = 5; i < thWs.length; i++) { document.getElementById(thWs[i]).disabled = true; };
  };
  if (str.includes("SONS OF BEHEMAT") === true) {
    for (i = 0; i < 5/*ohWs.length*/; i++) { document.getElementById(ohWs[i]).disabled = true; };
    for (i = 0; i < 5/*thWs.length*/; i++) { document.getElementById(thWs[i]).disabled = true; };
    for (i = 1; i < shis.length; i++) { document.getElementById(shis[i]).disabled = true; };
  };
};
...
function clickWea(checkId) {  
  ...
  if (document.getElementById("shi_01").checked == true) { addToWS(Trophy_strapped_Shield, "abilities", currentTab); };
  if (document.getElementById("shi_03").checked == true) { addToWS(Stonehorn_Shield, "abilities", currentTab); };
  ...
  if (checkId.substring(0,3) == "ohW") {    
    ...
    if (bStar == true && checkBox.checked == true) { 
      shi += 1;
      if (checkId == "shi_02") { changeDmgTable(actWeaName, addTimes, "add"); }; } 
      ...
      if (checkBox.checked == true) {
        shi += 1;
        if (checkId == "shi_02") { document.getElementById("orgSave0" + currentTab).innerHTML = 1; };
      } else {
        shi -= 1;
...
function resetCheckStep5_2() {
  ...
  if (document.getElementById("gaB_01").checked != true) {
    ...
    document.getElementById("bea_07").disabled = true;
  ...
  if (document.getElementById("miB_01").checked == true) {
    document.getElementById("bea_08").disabled = false;
    document.getElementById("bea_13").disabled = false;
    document.getElementById("bea_05").disabled = true;
  } else {
    document.getElementById("bea_08").disabled = true;
    document.getElementById("bea_13").disabled = true;
  };;
  var str = document.getElementById("heroKeys").innerHTML
...
function resetRestrictionsStep5() {
  var str = document.getElementById("heroKeys").innerHTML
  if (str.includes("MEGA-GARGANT") === true) {
...
function addCharEnh(checkId) {
  ...
  switch (enhancement) {
    ...
    case "Tyrannical Strength":  // to wound
      statCol = 4;
      if (checkBox.id.endsWith("b")) { 
        actWeaName = wea02nam;
        actStat = wea02wnd.search("blackstar");      // check if blackstar
...
function checkAura() {
  ...
  if (cellV >= 4) { checkAura.disabled = false; }
    ...
    if (checkAura.checked == true) {
      ...
      remFromWS(Unstoppable_Momentum, 'abilities');   // 
...
function resetRestrictionsStep6() {
  ...
  if (document.getElementById("moB_01").checked == true || document.getElementById("gaB_01").checked == true) { 
...
function show_hide(handler) {
  forms = ["commandAbilitiesForm", "bigNamesForm", "titanTraitsForm", /*"spellsForm",*/ "weaponsForm", /*"archetypeForm",*/ "beastsForm", "beastsAbiForm", "abilitiesForm", "spiritHostForm"]
    for (f = 0; f < forms.length; f++) {
    var hideCol = document.getElementById(forms[f]).getElementsByTagName("tr")
